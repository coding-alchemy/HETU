...
#
# Request/tool attribution and settlement semantics (verified read-only
# against the local client db, 2026-09-24):
#
# - ``model_usage.assistant_message_id`` is unique per request row and every
#   non-null value exists in ``message.id`` (31933/31933); a tool call is
#   attributed through ``part`` rows of that message whose
#   ``data.callID`` equals ``tool_usage.tool_call_id`` (42519/42519 tool rows
#   resolve exactly one such part). ``tool_call_count`` is only an unreliable
#   counter (under-reports on error rows) and is never used for attribution.
# - ``status`` is an execution outcome only, not a settlement state: every
#   completed row (31862) carries ``raw_usage_json`` whose values equal the
#   row columns, while every error (97) and cancelled (54) row has it NULL
#   with placeholder zero columns. Settlement evidence therefore is a
#   terminal status (``completed``/``error``/``cancelled`` — a ``running``
#   row's raw is only an intermediate snapshot and is refused) plus a
#   ``raw_usage_json`` that parses and field-wise equals the row; without
#   it usage is unknown (never zero) and the export is refused.
# - ``attempt_index`` distinguishes retries of one ``logical_request_id``;
#   the exported ``message_id`` encodes ``[logical_request_id, attempt_index]``
#   so retries meter separately and duplicate snapshots of one attempt still
#   dedupe by identity. Two rows for the same attempt with differing
#   status/model/token values are a conflict and refuse the export before
#   anything is published.
# - ``query_source`` is optional: when present its distribution is recorded
#   in provenance (transparency only, never a per-role token reconciliation);
#   when absent it is marked unavailable, never a zero distribution.
...
class _ZcodeDbExportError(Exception):
    """Controlled refusal: request identity, tool attribution or usage
    settlement cannot be verified, so no complete export may be published."""


KNOWN_MODEL_USAGE_STATUSES = frozenset(
    {"running", "completed", "error", "cancelled"}
)

# Only a terminal state can carry settlement evidence; a ``running`` row's
# raw_usage_json is an intermediate snapshot, never the final settlement.
TERMINAL_MODEL_USAGE_STATUSES = frozenset({"completed", "error", "cancelled"})

# row column -> raw_usage_json key; settlement is verified field-wise
_RAW_USAGE_KEYS = (
...
def _zcode_db_events(
    ...
    query_source_available: bool = False,
    ...
    Tool attribution follows the verifiable identity chain
    request.``assistant_message_id`` -> tool ``part.message_id`` ->
    ``part.data.callID`` == ``tool_usage.tool_call_id`` — never row order,
    counts or turn proximity. An orphan, duplicate or ambiguous tool, or a
    windowed tool owned by a request outside the window, raises
    :class:`_ZcodeDbExportError` with a locatable reason; so does any
    request whose usage settlement (terminal status plus a
    ``raw_usage_json`` matching the row field-wise) cannot be verified,
    and any attempt identity carried by two rows with differing
    status/model/token values. Duplicate snapshot rows of one attempt
    (identical values) dedupe by identity only after each row's own
    ...
    select_source = ", query_source" if query_source_available else ""
    rows = conn.execute(
        "SELECT logical_request_id, model_id, input_tokens, output_tokens, "
        "cache_read_input_tokens, cache_creation_input_tokens, "
        "started_at, completed_at, assistant_message_id, attempt_index, "
        "status, raw_usage_json" + select_source +
        " FROM model_usage WHERE session_id = ? AND started_at >= ?" + upper +
        " ORDER BY started_at",
        params,
    ).fetchall()
    if not rows:
    ...
    query_source_distribution: dict[str, int] = {}
    seen_identities: dict[tuple[str, int], tuple[Any, ...]] = {}
    for row in rows:
        (request_id, model, input_tokens, output_tokens, cache_read,
         cache_write, started_at, completed_at, amid, attempt, status,
         raw) = row[:12]
        query_source = row[12] if query_source_available else None
        if not isinstance(request_id, str) or not request_id:
            raise _ZcodeDbExportError(
                f"session {session_id}: model_usage row carries no usable "
                "logical_request_id; the request identity is unknown"
            )
        if not _valid_int(attempt) or attempt < 0:
            raise _ZcodeDbExportError(
                f"session {session_id}: request {request_id} carries "
                f"attempt_index {attempt!r} (not a non-negative integer); "
                "retry identity is unverifiable"
            )
        identity = f"{request_id} attempt {attempt}"
        if status not in KNOWN_MODEL_USAGE_STATUSES:
            raise _ZcodeDbExportError(
                f"session {session_id}: {identity} carries unknown status "
                f"{status!r}; the execution-state vocabulary drifted"
            )
        if status not in TERMINAL_MODEL_USAGE_STATUSES:
            raise _ZcodeDbExportError(
                f"session {session_id}: {identity} is still {status!r}; its "
                "raw_usage_json is only an intermediate snapshot, not final "
                "settlement evidence — refusing a complete export"
            )
        # Settlement is verified for EVERY row before any dedup decision:
        # a duplicate snapshot with corrupt evidence must refuse regardless
        # of row order.
        if raw is None:
            raise _ZcodeDbExportError(
                f"session {session_id}: {identity} (status {status}) has no "
                "settlement evidence (raw_usage_json is NULL); its usage is "
                "unknown and unknown is never zero — refusing a complete export"
            )
        try:
            settled = json.loads(raw)
        except (TypeError, json.JSONDecodeError) as error:
            raise _ZcodeDbExportError(
                f"session {session_id}: {identity} carries an unreadable "
                f"raw_usage_json ({error}); settlement cannot be verified"
            ) from error
        token_values: dict[str, int] = {}
        for column, key in _RAW_USAGE_KEYS:
            row_value = {
            ...
            raw_value = settled.get(key) if isinstance(settled, dict) else None
            if not (_valid_int(row_value) and _valid_int(raw_value)
                    and row_value >= 0 and raw_value >= 0
                    and row_value == raw_value):
                raise _ZcodeDbExportError(
                    f"session {session_id}: {identity} row {column}="
                    f"{row_value!r} contradicts raw_usage_json {key}="
                    f"{raw_value!r}; settlement cannot be verified"
                )
            token_values[column] = row_value
        signature = (
            status, model, input_tokens, output_tokens, cache_read, cache_write,
        )
        previous = seen_identities.get((request_id, attempt))
        if previous is not None:
            if previous == signature:
                continue  # verified duplicate snapshot dedupes by identity
            raise _ZcodeDbExportError(
                f"session {session_id}: {identity} appears with conflicting "
                "usage rows (status/model/token values differ); one attempt "
                "cannot have two settlements — refusing before publish"
            )
        seen_identities[(request_id, attempt)] = signature
        requests.append(
            {
                "request_id": request_id,
                "attempt": attempt,
                "amid": str(amid) if amid is not None else None,
                "model": model,
        ...
        if query_source_available:
            source_key = str(query_source)
            query_source_distribution[source_key] = (
                query_source_distribution.get(source_key, 0) + 1
            )

    # Request identity index over the WHOLE session (read-only): out-of-window
    # rows may prove an owner identity but never join the exported set.
    # Ownership ambiguity is judged by DISTINCT request identities
    # (logical_request_id, attempt_index): duplicate snapshots of one attempt
    # pointing at the same message are one owner, not many.
    owner_rows = conn.execute(
        "SELECT assistant_message_id, logical_request_id, attempt_index "
        "FROM model_usage "
        "WHERE session_id = ? AND assistant_message_id IS NOT NULL",
        (session_id,),
    ).fetchall()
    owner_identities: dict[str, set[tuple[str, Any]]] = {}
    for amid_value, owner_request, owner_attempt in owner_rows:
        owner_identities.setdefault(str(amid_value), set()).add(
            (str(owner_request), owner_attempt)
        )
    owner_count: dict[str, int] = {
        key: len(identities) for key, identities in owner_identities.items()
    }
    windowed_by_amid: dict[str, dict[str, Any] | None] = {}
    for request in requests:
        amid = request["amid"]
        if amid is None:
            continue
        # None marks a duplicate inside the window: attribution ambiguous
        windowed_by_amid[amid] = (
            request if amid not in windowed_by_amid else None
        )
    ...
    unreadable_parts = conn.execute(
        "SELECT count(*) FROM part WHERE session_id = ? AND json_valid(data) = 0",
        (session_id,),
    ).fetchone()[0]
    if unreadable_parts:
        raise _ZcodeDbExportError(
            f"session {session_id}: {unreadable_parts} part row(s) carry "
            "unreadable data; tool ownership cannot be verified"
        )
    call_owners: dict[str, set[str]] = {}
    for message_id, call_id in conn.execute(
        "SELECT message_id, json_extract(data, '$.callID') FROM part "
        "WHERE session_id = ? AND json_extract(data, '$.type') = 'tool'",
        (session_id,),
    ):
        if call_id:
            call_owners.setdefault(str(call_id), set()).add(str(message_id))

    tool_rows = conn.execute(
        "SELECT tool_call_id, tool_name, started_at FROM tool_usage "
        "WHERE session_id = ? AND started_at >= ?" + upper +
        " ORDER BY started_at",
        params,
    ).fetchall()
    seen_calls: set[str] = set()
    for tool_call_id, tool_name, _started in tool_rows:
        call_id = str(tool_call_id)
        if call_id in seen_calls:
            raise _ZcodeDbExportError(
                f"session {session_id}: tool_call_id {call_id} appears twice "
                "in the window; tool identity is ambiguous"
            )
        seen_calls.add(call_id)
        owners = call_owners.get(call_id)
        if not owners:
            raise _ZcodeDbExportError(
                f"session {session_id}: tool {call_id} ({tool_name}) has no "
                "owning tool part; attribution is unverifiable and the tool "
                "must not be dropped silently"
            )
        if len(owners) > 1:
            raise _ZcodeDbExportError(
                f"session {session_id}: tool {call_id} ({tool_name}) is "
                f"claimed by {len(owners)} messages; attribution is ambiguous"
            )
        owner_message = next(iter(owners))
        if owner_message not in message_ids:
            raise _ZcodeDbExportError(
                f"session {session_id}: tool {call_id} resolves to message "
                f"{owner_message} which has no message row; attribution is "
                "contradictory"
            )
        if owner_count.get(owner_message, 0) == 0:
            raise _ZcodeDbExportError(
                f"session {session_id}: tool {call_id} resolves to message "
                f"{owner_message} which no model_usage request admits; "
                "attribution is unverifiable"
            )
        if owner_count[owner_message] > 1:
            raise _ZcodeDbExportError(
                f"session {session_id}: tool {call_id} resolves to message "
                f"{owner_message} shared by multiple requests; attribution "
                "is ambiguous"
            )
        owner = windowed_by_amid.get(owner_message)
        if owner is None:
            raise _ZcodeDbExportError(
                f"session {session_id}: tool {call_id} ({tool_name}) belongs "
                f"to request message {owner_message} outside the export "
                "window; the tool cannot be metered without its request and "
                "must not be attached to an in-window request by guessing"
            )
        owner["tools"].append((call_id, str(tool_name)))
    ...
    for request in requests:
        tool_calls = [
            ...
            for call_id, tool_name in request["tools"]
        ]
        event: dict[str, Any] = {
            "source": "zcode-db",
            "session_id": session_id,
            ...
            "cached_input_read": request["tokens"]["cache_read_input_tokens"],
            "cached_input_write": (
                request["tokens"]["cache_creation_input_tokens"]
            ),
            "input_includes_cache": True,
            "model": request["model"],
            "captured_at": now,
        }
        started_iso = _ms_to_iso(request["started_at"])
        completed_iso = _ms_to_iso(request["completed_at"])
        if started_iso:
    ...
    info = {
        ...
        "query_source_distribution": (
            dict(sorted(query_source_distribution.items()))
            if query_source_available
            else None
        ),
    }
...
def _cmd_check(args: argparse.Namespace) -> int:
    ...
    if not timing["complete"]:
        ...
        if not timing["gaps"]:
            # 无具体 gap 的不完整 = 边界时刻缺失（None 不是非法时间戳，
            # summarize_timing 不产生 gap）；complete=False 本身就足以
            # 阻止通过，未知不为零，原因须可定位到缺失字段。
            missing = [
                label
                for label, value in (
                    ("request_at", meta.get("request_at")),
            ...
            failures.append(
                f"timing incomplete: {', '.join(missing) or 'total_wait'} "
                "unknown; the request->delivery wait is not established "
...
def _cmd_db_export(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"unsupported: 数据库不可读：{error}", file=sys.stderr)
        return 1
    conn.isolation_level = None  # explicit transaction control (snapshot read)
    try:
        with contextlib.closing(conn):
            # Phase 01 requirement: every query below reads ONE consistent
            # database snapshot. A deferred read transaction pins the view;
            # a concurrent commit surfaces as sqlite3.Error and fails the
            # export instead of mixing pre/post-update rows into one result.
            conn.execute("BEGIN")
            missing = _zcode_db_check_schema(conn)
            if missing:
                print(
            ...
            if len(args.scope) != len(args.session):
                ...
                return 2
            # 6-A4: refuse before writing when any prior output exists — a
            # complete earlier export or a half-written one is never appended
            # to, overwritten or mixed into this run's events.
            events_path = evidence_dir / "usage-events.jsonl"
            provenance_path = evidence_dir / "db-export.json"
            for existing in (events_path, provenance_path):
                if existing.exists():
                    print(
                        f"refusing to export: {existing} already exists; "
                        "db-export never appends to or overwrites prior "
                        "output — choose a fresh observation directory",
                        file=sys.stderr,
                    )
                    return 2
            query_source_available = "query_source" in {
                row[1] for row in conn.execute("PRAGMA table_info(model_usage)")
            }
            # Phase 1: collect and verify every (scope, session); a failure
            # anywhere means nothing is written at all.
            collected: list[
                tuple[str, str, list[dict[str, Any]], dict[str, Any]]
            ] = []
            for scope, session in zip(args.scope, args.session, strict=True):
                try:
                    result = _zcode_db_events(
                        conn,
                        session_id=session,
                        scope=scope,
                        request_at_ms=request_at_ms,
                        window_end_ms=window_end_ms,
                        query_source_available=query_source_available,
                    )
                except _ZcodeDbExportError as error:
                    print(f"db-export refused: {error}", file=sys.stderr)
                    return 1
                if result is None:
                    print(
                    ...
                    return 1
                events, info = result
                collected.append((scope, session, events, info))
            conn.execute("ROLLBACK")  # all reads done; release the snapshot
            ...
            written = 0
            for scope, session, events, info in collected:
                provenance["sessions"].setdefault(scope, {})[session] = info
                written += len(events)
            # Phase 2: publish. On a write error, remove only files this run
            # created (both were verified absent above); pre-existing evidence
            # is never touched.
            created: list[Path] = []
            try:
                if any(events for _s, _ss, events, _i in collected):
                    # the first append creates this file; it is this run's
                    # product even if a later write fails
                    created.append(events_path)
                for _scope, _session, events, _info in collected:
                    for event in events:
                        append_event(events_path, event)
                # Register before publishing: _write_json can publish the
                # target and then fail on temp-file cleanup, and the cleanup
                # below must still know the target is this run's product.
                created.append(provenance_path)
                try:
                    _write_json(provenance_path, provenance)
                except FileExistsError:
                    # the target is not this run's product — never remove it
                    created.remove(provenance_path)
                    raise
                ...
                for created_path in created:
                    with contextlib.suppress(OSError):
                        created_path.unlink()
                print(
                    f"db-export write failed: {error}; removed the partial "
                    "output created by this run (pre-existing evidence "
                    "untouched)",
                    file=sys.stderr,
                )
                return 1
