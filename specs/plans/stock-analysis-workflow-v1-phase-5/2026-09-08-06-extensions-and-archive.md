...
> 文档版本：v1.4
> 文档状态：已批准；06.1／06.3 已执行待提交，06.2 四宿主真实会话验收待人工执行；
...
- [x] 先加入以下无损未知字段和只读测试，以及多次尝试／原采用指针／用户决定 fixture。
...
- [x] 读取 `state.json`、`stage_results/*.json` 和已有报告，已知请求、证据／主张、论点、
  阶段尝试、原采用和用户决定逐字段呈现；已有状态标“原记录”，不能当新任务恢复状态。
- [x] 只检查源目录内的引用；缺引用、损坏 JSON 或未知版本标具体范围并保留可取回原文件。
  不执行 HTML、命令、反序列化代码或外链下载，不猜测和重建研究结论。
- [x] 导出到新目标，生成可读索引及授权允许的原记录／附件，索引保留来源与字段定位。
  目标已存在、路径穿越、symlink 越界都拒绝；输出不得位于源内部或覆盖源文件。
- [x] 受限材料保留允许定位及原因；原记录含秘密时不复制到索引／包，原件保留原位置，
  明示脱敏和未导出范围。测试使用无价值模拟 secret，不能用真实凭据检验。
- [x] 测试已知完整字段、多版本未知字段、多尝试、缺引用、坏 JSON、危险路径、已有目标和
