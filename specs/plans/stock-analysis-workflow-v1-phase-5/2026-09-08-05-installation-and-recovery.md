...
> 文档版本：v1.5
> 文档状态：已批准；2026-09-08 任务 05.1 执行完毕（33/33 安装器测试+全量门禁通过）；
> 2026-09-12 任务 05.2/05.3 执行完毕并修复三轮评审确认的全部问题（组合恢复闭合、
> 受管归属证据、共享安装保护、旧版 hetu-stock/venv 布局升级、回滚 --host 与如实
> 报告、记录提前写入与版本对应、删除仅记录证据、多宿主环境复用、切换前持久化与
> 中断恢复、备份↔组合显式关联回滚、宿主引用环境保护；定向 111/111、全量门禁
> 1374 通过；证据见
> .hetu/validation/phase-5/20260912-stage05-installation/record.md；
> Linux/ext4 实测未验证，由 07 补齐）
> 2026-09-12 最后一项复评修复：发布后验证失败统一复用组合回退，恢复旧组合的
> finalized 记录；已验证失败恢复后再次更新成功，3 项直接回归及全量门禁
> （1374 项测试）通过；最后阻断项关闭。
> 批准日期：2026-09-08
...
- [x] 在 `test_install_script.py` 的受控 HOME／XDG fixture 中先测试 pip 失败、CLI 自检失败、
  非受管启动器、Skill 切换后启动器失败；预期旧环境、旧启动器和旧 Skill 保持或恢复一致。
  环境变量仅由测试子进程传入，不能改执行者全局配置。
  （2026-09-12：5 个新失败注入测试先行失败，重写 install.sh 后通过。）
- [x] 修改 install.sh：在最终保留的版本目录创建新 venv，完成依赖安装和 CLI 自检，不升级
  旧 venv，也不在创建后移动 venv；保留 Python 3.11／3.12、TLS 与受管目录限制。
  （新环境路径 `envs/env-<时间戳>-<pid>`；自检含 `--help` 与 `skill validate` 源包。）
- [x] 通过同一维护事务关联旧新环境与 Skill；启动器用临时链接原子替换，拒绝覆盖非受管文件。
  Skill 发布与启动器切换分步，各自原子；后续步骤失败恢复已切换部分，不虚称跨目录整体原子。
  （切换后验证失败时恢复旧启动器并删除废弃新环境，见
  test_post_switch_failure_restores_previous_launcher。）
- [x] 成功前不删除旧组合，成功后保留上一可用组合；回滚沿用同一校验、交换和启动器流程。
  多宿主仍引用共享环境时不能删除它。
  （成功后仅保留当前+上一环境；共享环境拒绝删除见 test_skill_uninstall_keeps_shared_environment。）
- [x] 用真实临时版本环境核对 CLI 可运行及 shebang 仍指向存在的解释器；无完整辅助包的
  public 基础研究仍可使用 Skill 和原生工具，日常研究不自动跑安装和工程检查。
  （2026-09-12 真实临时 HOME 冒烟：真实 venv+pip，shebang 解释器存在、CLI 可运行。）

## 5. 任务 05.3：维护命令、ZCode 路径与真实平台验证

- [x] CLI 增 status／diagnose：只读完整性、备份、受管启动器和辅助能力，输出脱敏定位、
  原因和下一动作，不自动联网、修复或列环境变量全集。保留顶层仅 `skill`／`helper`。
- [x] rollback 使用明确完整备份走 05.1／05.2，保留被换下版本；无备份或恢复失败非零退出。
  uninstall 先列所选 Skill、受管启动器、环境与扩展绑定范围，再按用户已授权操作；共享环境、
  其他宿主、研究、旧报告、授权配置和非受管文件保留，不把整个宿主目录当删除目标。
  （`--yes` 为用户确认；无 `--yes` 列范围并以退出码 2 结束；扩展绑定本期无（阶段 06）。）
- [x] 使用 01 的 ZCode 原生发现证据实现 default_user_skill_root 分支、install.sh host 解析
  和帮助；路径未验证则明确失败或要求用户给出明确 destination，不能宣称自动发现已支持。
  新路径不得改变原 Codex、Claude、OpenCode 默认值及自定义路径含义。
  （installer 分支 05.1 已落地，install.sh zcode 解析与帮助 2026-09-12 补齐并测试。）
- [x] 在 macOS/APFS 和 Linux/ext4 实际执行交换、并发、中断后恢复及不支持文件系统负例。
  单元 mock 只验证错误路径，不替代真实平台成功；无平台时登记未验证，由 07 补齐。
  （macOS/APFS 已实测：交换成功路径走真实 renameatx_np、并发 flock 串行化、
  中断恢复与 ENOSYS 负例在 05.1 测试内执行，另做真实临时 HOME 端到端冒烟；
  Linux/ext4 无宿主，登记未验证，未记为支持。）
- [x] 更新使用说明的安装、诊断、回滚、卸载及平台条件，声明实际支持边界；暂存／备份不被
  Skill 自动发现，真实安装入口继续是实体目录。
  （docs/agent-skill-usage.md 与 README.md 已更新；维护区位于宿主发现目录之外。）
