...
# --- Phase-5 前半全面评审 R1–R3：回滚与卸载安全 ----------------------------


def test_rollback_refuses_to_overwrite_unmanaged_launcher(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """评审 R1：启动器位置被用户换成普通文件时，回滚必须在交换前拒绝，
    用户文件与当前安装都保持不变；受管启动器的正常回滚不受影响。"""
    managed = _fake_managed_home(tmp_path, monkeypatch)
    first = tmp_path / "v1" / "hetu-stock-analysis"
    _make_versioned_package(first, "version-1")
    codex_root = tmp_path / "home/.codex/skills"
    ...
    _point_launcher(managed["launcher"], managed["current"])
    managed["launcher"].unlink()
    managed["launcher"].write_text("USER OWNED SCRIPT\n", encoding="utf-8")

    result = runner.invoke(app, ["skill", "rollback", "--host", "codex"])

    assert result.exit_code != 0
    assert "unmanaged" in result.output.lower()
    # 用户文件字节不变，当前安装（v2 Skill 与 v2 环境组合）不变
    assert not managed["launcher"].is_symlink()
    assert managed["launcher"].read_text(encoding="utf-8") == "USER OWNED SCRIPT\n"
    marker = codex_root / "hetu-stock-analysis" / "references" / "version-marker.txt"
    assert marker.read_text(encoding="utf-8") == "version-2"


def test_uninstall_remove_env_honors_persistent_host_reference(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """评审 R2：卸载另一宿主时，持久登记的宿主引用（自定义 CODEX_HOME
    安装）必须计入共享核对；当前会话没有该环境变量也不能删除仍被引用
    的共享环境与启动器。"""
    managed = _fake_managed_home(tmp_path, monkeypatch)
    source = tmp_path / "v1" / "hetu-stock-analysis"
    _make_versioned_package(source, "version-1")
    codex_home = tmp_path / "custom-codex"
    monkeypatch.setenv("CODEX_HOME", str(codex_home))
    installed = runner.invoke(
        app, ["skill", "install", "--host", "codex", "--source", str(source)]
    )
    assert installed.exit_code == 0, installed.output
    host_ref = managed["managed_root"] / "hosts" / "codex.json"
    assert host_ref.is_file()
    assert str(managed["current"]) in host_ref.read_text(encoding="utf-8")
    install_skill(source, tmp_path / "home/.claude/skills")
    monkeypatch.delenv("CODEX_HOME")

    result = runner.invoke(
        app, ["skill", "uninstall", "--host", "claude", "--yes", "--remove-env"]
    )

    assert result.exit_code != 0
    assert "shared" in result.output.lower()
    # 失败先于任何删除：双方 Skill、共享环境、启动器与登记引用全部保留
    assert (tmp_path / "home/.claude/skills/hetu-stock-analysis").is_dir()
    assert (codex_home / "skills/hetu-stock-analysis").is_dir()
    assert managed["current"].is_dir()
    assert managed["launcher"].is_symlink()
    assert host_ref.is_file()


def test_rollback_launcher_failure_restores_consistent_combination(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """评审 R3：回滚后段（启动器切换）失败必须恢复交换前的一致组合并
    报告失败；解除故障后重试仍完成用户原本要求的回滚，而不是把上一
    失败产生的备份误当目标。"""
    managed = _fake_managed_home(tmp_path, monkeypatch)
    first = tmp_path / "v1" / "hetu-stock-analysis"
    _make_versioned_package(first, "version-1")
    ...
    native_replace = installer_module.os.replace

    def fail_launcher(src: str, dst: str) -> None:
        if Path(dst) == managed["launcher"]:
            raise PermissionError("injected launcher replace failure")
        native_replace(src, dst)

    monkeypatch.setattr(installer_module.os, "replace", fail_launcher)
    result = runner.invoke(app, ["skill", "rollback", "--host", "codex"])

    assert result.exit_code == 1
    # 恢复交换前的一致组合：Skill 仍是 v2，启动器仍指向 v2 环境
    assert marker.read_text(encoding="utf-8") == "version-2"
    assert managed["launcher"].resolve() == (
        managed["current"] / "bin" / "hetu-stock"
    ).resolve()

    monkeypatch.setattr(installer_module.os, "replace", native_replace)
    retry = runner.invoke(app, ["skill", "rollback", "--host", "codex"])

    assert retry.exit_code == 0, retry.output
    # 重试完成原本要求的回滚：v1 Skill 与 v1 环境组合
...
def test_rollback_restore_failure_stays_unfinished_until_recovered(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """评审 R3 双故障：启动器切换失败后，恢复交换也失败时，事务不得记为
    完成；两份现场保留，失败现场不得被后续维护当作普通完整备份；解除
    故障后重试先完成恢复（回到交换前一致组合），再完成用户原本要求的
    回滚，而不是把失败现场误当备份并成功停留在错误版本。"""
    managed = _fake_managed_home(tmp_path, monkeypatch)
    first = tmp_path / "v1" / "hetu-stock-analysis"
    _make_versioned_package(first, "version-1")
    codex_root = tmp_path / "home/.codex/skills"
    ...
    native_replace = installer_module.os.replace
    native_exchange = installer_module.exchange_directories
    exchange_calls = 0
    ...
    def fail_restore(src: Path, dst: Path) -> None:
        nonlocal exchange_calls
        exchange_calls += 1
        if exchange_calls == 2:
            raise PermissionError("injected restoration exchange failure")
        native_exchange(src, dst)

    monkeypatch.setattr(installer_module.os, "replace", fail_launcher)
    monkeypatch.setattr(installer_module, "exchange_directories", fail_restore)
    result = runner.invoke(app, ["skill", "rollback", "--host", "codex"])

    assert result.exit_code == 1
    # 双故障现场：Skill 已交换到 v1，启动器仍指向 v2 环境；两份现场都保留
    assert marker.read_text(encoding="utf-8") == "version-1"
    assert managed["launcher"].resolve() == (
        managed["current"] / "bin" / "hetu-stock"
    ).resolve()
    area = installer_module._read_area(codex_root)
    assert area is not None
    record = area.read_record()
    assert record is not None
    # 恢复本身失败不能提前记为完成（既非 finalized 也非正常 rolled_back）
    assert record["state"] not in {"finalized", "rolled_back"}
    pending = Path(str(record["staging"])).parent
    # 恢复中的备份仍持有交换前的 v2，失败现场两份版本都保留
    pending_marker = (
        pending / "hetu-stock-analysis" / "references" / "version-marker.txt"
    )
    assert pending_marker.read_text(encoding="utf-8") == "version-2"
    # 恢复中的现场不得被列为普通可用备份
    listed = {info.path for info in installer_module.list_skill_backups(codex_root)}
    assert pending not in listed

    monkeypatch.setattr(installer_module.os, "replace", native_replace)
    monkeypatch.setattr(installer_module, "exchange_directories", native_exchange)
    retry = runner.invoke(app, ["skill", "rollback", "--host", "codex"])

    # 解除故障后的安全行为：维护先完成恢复，再完成原本要求的 v1 回滚；
    # 绝不把失败现场当作普通备份而“成功”停留在 v2/v2
    assert retry.exit_code == 0, retry.output
    assert marker.read_text(encoding="utf-8") == "version-1"
    assert managed["launcher"].resolve() == (
        managed["previous"] / "bin" / "hetu-stock"
