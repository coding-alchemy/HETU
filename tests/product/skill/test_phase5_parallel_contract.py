...
def test_formal_dispatch_and_redispatch_pass_actual_reuse_switch_and_boundaries() -> None:
    dispatch_raw = _dispatch_section()
    dispatch = _flow(dispatch_raw)

    # 正式派发入口（“派发前”固定段、十二项输入之前）明确传递主任务已确定的实际开关与边界
    assert dispatch.index("派发前主Agent统一固定") < dispatch.index(
        "首次派发与重派"
    ) < dispatch.index("每个子任务的输入自足")
    assert "实际`reuse_previous_task_data`开关及对应边界" in dispatch
    assert "不得以默认值替代主任务已确定的选择" in dispatch

    # false 不被默认 true 替代：子任务同样遵守关闭边界，本任务共享输入仍可用
    assert "不查找、不读取、不采用旧任务资料" in dispatch
    assert "上下文中的旧结论" in dispatch
    assert "共享输入仍按本任务共享取数规则可用" in dispatch

    # 恢复规则的传递范围覆盖临时及正式子任务（含重派）与交付前独立核验，既有覆盖保留
    boundary = _flow(_section(_read(RECOVERY), "## 关闭复用的上下文边界"))
    assert "实际开关与对应禁用边界必须传递" in boundary
    assert "临时研究子任务" in boundary
    assert "正式子任务（含首次派发与重派）" in boundary
    assert "交付前独立核验" in boundary
    assert "独立上下文”不自动视为无旧资料" in boundary

    # 预检的传递禁用边界范围同步覆盖正式子任务
    precheck = _flow(_section(_read(HOST_TOOLS), "## 研究前能力预检"))
    assert "能否向临时子任务、正式子任务（含重派）与交付前独立核对传递禁用边界" in precheck
