...
def _read_registry(root: Path | None = None) -> dict[str, Any]:
    ...
    _check_registry_shape(data, path)
    return data


def _check_registry_shape(data: dict[str, Any], path: Path) -> None:
    """在任何消费或写回前校验登记表形态；坏表只拒绝，不自动修复。

    缺省 bindings 与缺省宿主绑定集合仍按既有语义初始化为空对象。
    """
    extensions = data["extensions"]
    if not isinstance(extensions, dict):
        raise ExtensionError(f"登记表格式无效: extensions 必须是对象 ({path})")
    for ext_id, entry in extensions.items():
        location = f"extensions.{ext_id}"
        if not isinstance(entry, dict):
            raise ExtensionError(f"登记表格式无效: {location} 必须是对象 ({path})")
        versions = entry.get("versions")
        if not isinstance(versions, dict):
            raise ExtensionError(
                f"登记表格式无效: {location}.versions 必须是对象 ({path})"
            )
        if not versions:
            raise ExtensionError(
                f"登记表格式无效: {location}.versions 为空，无法选择默认版本 ({path})"
            )
        for version, record in versions.items():
            if not isinstance(record, dict):
                raise ExtensionError(
                    f"登记表格式无效: {location}.versions.{version} 必须是对象 ({path})"
                )
            _check_version_record_fields(record, f"{location}.versions.{version}", path)
    bindings = data.setdefault("bindings", {})
    if not isinstance(bindings, dict):
        raise ExtensionError(f"登记表格式无效: bindings 必须是对象 ({path})")
    for host in HOSTS:
        bindings.setdefault(host, {})
    for host, host_bindings in bindings.items():
        location = f"bindings.{host}"
        if not isinstance(host_bindings, dict):
            raise ExtensionError(f"登记表格式无效: {location} 必须是对象 ({path})")
        for ext_id, binding in host_bindings.items():
            if not isinstance(binding, dict):
                raise ExtensionError(
                    f"登记表格式无效: {location}.{ext_id} 必须是含 version 的对象 ({path})"
                )
            try:
                _checked_version(binding.get("version"))
            except ExtensionError as exc:
                raise ExtensionError(
                    f"登记表格式无效: {location}.{ext_id}.version"
                    f" 不是合法版本字符串 ({path})"
                ) from exc


_LIST_FIELDS = ("capabilities", "provides", "requires")
_TEXT_FIELDS = ("path", "source", "summary", "compatibility")


def _check_version_record_fields(
    record: dict[str, Any], location: str, path: Path
) -> None:
    """校验下游实际读取的版本记录字段；缺失字段容忍，存在即须类型正确。

    null 视为「存在但类型错误」拒绝，避免下游 list()/比较出现未受控异常，
    也避免 update 先写回登记表再在读取旧记录时崩溃。
    """
    for field in _LIST_FIELDS:
        ...
        if not isinstance(value, list) or not all(
            ...
            raise ExtensionError(
                f"登记表格式无效: {location}.{field} 必须是字符串列表 ({path})"
            )
    if "files" in record:
        files = record["files"]
        if not isinstance(files, dict) or not all(
            ...
            raise ExtensionError(
                f"登记表格式无效: {location}.files 必须是字符串到字符串摘要的对象 ({path})"
            )
    for field in _TEXT_FIELDS:
        ...
        if not isinstance(record[field], str):
            raise ExtensionError(
                f"登记表格式无效: {location}.{field} 必须是字符串 ({path})"
            )
...
def _managed_extension_dir(root: Path, extension_id: str) -> Path:
    """卸载删除目标：登记表键必须仍是合法扩展 ID 且解析后留在受管根内。

    登记表可能被手改或损坏（例如外部绝对路径键），删除前必须重新核验，
    非法目标受控拒绝，不触达受管根外的任何文件。
    """
    if not _EXTENSION_ID.match(extension_id):
        raise ExtensionError(f"扩展 ID 形态非法，拒绝卸载删除: {extension_id!r}")
    root_resolved = root.resolve()
    target = (root_resolved / extension_id).resolve()
    if target != root_resolved / extension_id:
        raise ExtensionError(f"卸载目标越出受管目录，拒绝: {extension_id!r}")
    return target
...
def _version_sort_key(
    ...
    连续数字段按整数、非数字段按原字符序比较；固定类型标签（数字 0、
    文字 1）避免 int/str 混比。分段序列单独构成第一级键，保证前缀
    语义（1.0 < 1.0.1）；原字符串仅在分段完全相同时作最终平局键，
    保证全序确定（如 01.0 与 1.0 选 1.0）。不实现 SemVer 预发布
    优先级，也不改变版本字符串的合法范围。
    """
...
def _latest_version(versions: dict[str, Any]) -> str:
    if not versions:
        raise ExtensionError("扩展没有已登记版本，无法选择默认版本")
...
def _find_version(entry: dict[str, Any], version: str | None) -> tuple[str, dict[str, Any]]:
    ...
    if version is None:
        chosen = _latest_version(versions)
