                ...
                issues.append(
                    {
                        "file": relative,
                        "message": (
                            # 诊断说明是会进入导出索引的显示路径，只能用脱敏
                            # 后的显示数据，不能 repr 可能含秘密的原记录值。
                            f"未知版本 schema_version（原记录 "
                            f"{_strip_secrets(schema_version)!r}，"
...
def _render_index(result: dict[str, Any], redacted: list[dict[str, str]]) -> str:
    lines = [
        ...
        f"- schema_version：{_strip_secrets(result['schema_version'])!r}（按原记录，未迁移）",
        "",
        "## 字段定位",
        "",
    ]
    fields = result["fields"]
    request = fields.get("请求")
    if request is not None and not isinstance(request, dict):
        # 未知字段结构：不套用已知格式渲染，给出定位与无法映射说明。
        lines.append(
            "- 请求：字段结构无法映射，未按已知格式解读"
            "（原记录保留于源目录 state.json）"
        )
    else:
        # 索引只使用经脱敏的显示数据：已识别秘密不能通过 repr/格式化再次输出。
        safe_request = _strip_secrets(request) if isinstance(request, dict) else {}
        subject = safe_request.get("subject")
        symbol = subject.get("symbol") if isinstance(subject, dict) else None
        lines.append(
            f"- 请求：subject={symbol!r}，as_of={safe_request.get('as_of')!r}，"
            f"depth={safe_request.get('depth')!r}（state.json#request）"
        )
    decisions = fields.get("用户决定")
    if decisions is not None and not isinstance(decisions, list):
        lines.append(
            "- 用户决定：字段结构无法映射，未按已知格式解读"
            "（state.json#workflow_state.user_decisions）"
        )
    else:
        lines.append(
            f"- 用户决定：{len(decisions or [])} 条"
            "（state.json#workflow_state.user_decisions）"
        )
    attempts = fields.get("阶段尝试")
    if attempts is not None and not isinstance(attempts, dict):
        lines.append(
            "- 阶段尝试：字段结构无法映射，未按已知格式解读"
            "（state.json#stage_attempts）"
        )
    else:
        rendered_attempts = []
        for stage, items in sorted((attempts or {}).items()):
            count = str(len(items)) if isinstance(items, list) else "无法映射"
            rendered_attempts.append(f"S{stage}×{count}")
        lines.append(
            "- 阶段尝试："
...
def export_archive(source: Path, output: Path) -> dict[str, Any]:
    ...
    for path in sorted(source.rglob("*")):
        ...
        if actual_relative in restricted_relatives:
            ...
            continue
        # 安全判定针对解析后的实际内容与对象：别名、目录位置或文件后缀
        # 不能绕过。内容可解析为 JSON 且含已支持秘密字段时不复制；非 JSON
        # 内容不做秘密识别，按原样复制。
    ...
    # 零文件可复制（全部受限）时仍生成可读索引；目标在入口已确认不存在，
    # 此处只创建本次导出的新目录，不覆盖任何既有内容。
