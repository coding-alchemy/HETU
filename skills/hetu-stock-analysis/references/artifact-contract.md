...
目录名即 `run_id`，固定为 `<证券简称>-<证券代码>-<请求深度>-<任务时间>`：简称使用 W1 核验的
交易所公开简称，代码使用大写市场后缀，深度只能是 `quick`、`standard` 或 `deep`，`<任务时间>`
使用 W1 唯一核验后实际创建正式目录时的带时区 `YYYYMMDDTHHMMSS±HHMM`。W1 核验前不创建正式或
临时研究目录；候选任务卡保留在 Agent 当前上下文。W1 核验后创建研究根、四类 `artifacts/`
子目录、`manifest.json` 和 W0/W1 工作包文件，归档时使用
`.hetu/locks/<证券简称>-<证券代码>-<请求深度>-<任务时间>/lock-record.json` 复用同一 `run_id`。
同一任务恢复使用原目录；独立重试按新的实际任务时间创建新目录，不覆盖失败或已交付运行。
其他工作包首次开始时只创建自己的文件。
...
扩展加载记录（阶段 06 起的条件信息；旧记录缺这些字段只表示当时未安装扩展）：

- `run` 条件字段 `extensions`：非空数组，逐条记录本次实际加载的第三方工作包扩展
  `{id, version, source, summary, enabled_scope}`，五键齐全且均为非空字符串，`id` 形如
  `x.<命名空间>.<名称>`。只记录实际加载的包，不记录未启用、校验失败或本宿主禁用的候选。
- 条目 `work_package` 允许使用已在本次 `run.extensions` 记录的 `x.*` ID；未记录的 `x.*` ID
  按 manifest 结构错误处理。核心 W0-W10 包不得被扩展条目替代或代写。
- 机械边界：检查器只认可本次 manifest 的显式记录，不扫描任何全局插件目录补造身份，
  不判断扩展内容的语义适用性，也不因扩展存在而放宽核心包的固定要求。
...
口径或期间修正时，在同一次修正中把被替代条目从 `adopted` 改为 `superseded`，把真实失败保留为
`failed`，并同步新产物的实际输入链和所有直接引用；不得只新增正确文件而让旧错误产物继续采用。
...
调用既有 canonical 工具不等于创建脚本。调用记录不得放入 `artifacts/scripts/`，不得登记为 `type=script`；
只有本次运行实际创建或修改的可执行代码文件才按脚本留存。调用记录需要单独留存时，
写为具有实际输入链的 `derived` 产物，在记录正文中保存 canonical 工具哈希和每组输入与输出，并由
manifest 登记该记录文件及其全部本地输入。
